/*************
DARPA AIMEE - HECTOR source code CWE-416 pipeline
...
namespace {
  ...
  struct HECTOR_416 : public ModulePass {
    static char ID;
    HECTOR_416() : ModulePass(ID) {}
	...
	bool runOnModule(Module &M) override {
    	errs() << "HECTOR (CWE-416) is starting.\n";
			...
			for (Function::iterator BB = func->begin(), BBE = func->end(); BB != BBE; ++BB){
				...
				for(BasicBlock::iterator I = block->begin(), IE = block->end(); I != IE; ++I){
					...
					// Call instructions have features to extract, and may also require tagging as root cause or manifestation points
						...
						// Tag the call as a root cause point if it is a call to a builtin free function
						...
						// If not a free call, tag it as a manifestation point if it has at least one argument.
						else if(cbi->arg_size() > 0 ){ node->tags = "\"manifestation\""; }
					}

					// Mark return type instructions as manifestation points if they return a pointer type
					else if(ReturnInst* ri = dyn_cast<ReturnInst>(instr)){
						Value* retval = ri->getReturnValue();
						if(retval != nullptr && retval->getType()->isPtrOrPtrVectorTy()){ node->tags = "\"manifestation\"";	}						
					}

					// Mark load instructions (pointer dereferences) as manifetation points
					...
					if(labels.size() != 0 && node->filename != ""){
						...
						for(Label lbl : labels){
							if(endsWith(sourceline, lbl.first)){
								...
								if(lbl.second == "freed_variable" && node->tags == "\"root_cause\"")
									node->labels = "\"" + lbl.second + "\"";
								else if(lbl.second == "use_after_free" && node->tags == "\"manifestation\"")
									node->labels = "\"" + lbl.second + "\"";
							}
						}
					}
		...
		*/
		...
		outfile << "] }\n";
		outfile.close();
		errs() << "HECTOR (CWE-416) is finished.\n" ;
		return false;
    }
  };
}

char HECTOR_416::ID = 0;
static RegisterPass<HECTOR_416> X("HECTOR_416", "HECTOR (CWE-416) Label Association and Feature Generation Pass");
