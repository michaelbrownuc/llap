/*************
DARPA AIMEE - HECTOR source code CWE-121/CWE-122 pipeline
...
namespace {
  ...
  struct HECTOR_121 : public ModulePass {
    static char ID;
    HECTOR_121() : ModulePass(ID) {}

    bool runOnModule(Module &M) override {
    	errs() << "HECTOR (CWE-121) is starting.\n";
		...
		for (Module::iterator F = M.begin(), E = M.end(); F != E; ++F){
			...
			for (Function::iterator BB = func->begin(), BBE = func->end(); BB != BBE; ++BB){
				...
				for(BasicBlock::iterator I = block->begin(), IE = block->end(); I != IE; ++I){
					...
					// Root cause points are the instantiation of statically sized memory buffers on the heap or stack
					else if(AllocaInst* ai = dyn_cast<AllocaInst>(instr)){
						Type* alloca_type = ai->getAllocatedType();
						if(alloca_type->isStructTy() || alloca_type->isArrayTy() || alloca_type->isVectorTy() || alloca_type->isPointerTy()){
					...
					// For Integer Overflow, potential manifestation points are function calls (and invocations) with arguments 
						...
						// Only tag the call if it has at least one argument, or is a call to a dynamic memory allocation function (malloc)
						if(node->function == "malloc") {node->tags = "\"root_cause\"";}
						else if(cbi->arg_size() > 0 ) { node->tags = "\"manifestation\""; }
					}

					// Mark load instructions (pointer dereferences) as manifetation points. Covers accessing class members, etc.
					else if(dyn_cast<LoadInst>(instr)) { node->tags = "\"manifestation\""; }

					// Mark store instructions (memory writes to a pointer value) as manifestation points. Covers indexes into arrays.
					else if(dyn_cast<StoreInst>(instr)) { node->tags = "\"manifestation\""; }
					...
					if(labels.size() != 0 && node->filename != ""){		
						...
						for(Label lbl : labels){
							if(endsWith(sourceline, lbl.first)){
								// Ensure that the label and tag correspond to eachother for manifestation points
								if(endsWith(lbl.second, "_overflow") && node->tags == "\"manifestation\"")
									node->labels = "\"" + lbl.second + "\"";
								else if(lbl.second == "declared_buffer"){
									// Apply to this instruction if there is a tag
									if(node->tags == "\"root_cause\"")
										node->labels = "\"" + lbl.second + "\"";

									// Special case for CWE121/122: need to back-propagate label to allocation instructions since the LLVM debug and 
									// lifetime intrinsics will be removed from graphs by hector_ml.
									// This assumes that the allocation node is processed before the debug declare instruction.
									if(DbgDeclareInst* ddi = dyn_cast<DbgDeclareInst>(instr)){
										if(AllocaInst* ai = dyn_cast<AllocaInst>(ddi->getAddress())){
											auto find_it = node_mappings.find(dyn_cast<Instruction>(ai));
											if(find_it != node_mappings.end())
												find_it->second->labels = "\"" + lbl.second + "\"";
											else 
												errs() << "    ERROR: Unable to back-propagate label to allocation instruction, it does not exist!\n";
										}
									}	
								}
							}
		...
		*/
		...
		outfile << "] }\n";
		outfile.close();
		errs() << "HECTOR (CWE-121) is finished.\n" ;
		return false;
    }
...
char HECTOR_121::ID = 0;
static RegisterPass<HECTOR_121> X("HECTOR_121", "HECTOR (CWE-121) Label Association and Feature Generation Pass");
